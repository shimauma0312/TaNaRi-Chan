...
export const useTodoList = (options: UseTodoListOptions = {}) => {
  ...
  const fetchTodos = useCallback(
    async (targetUserId?: string) => {
      ...
      try {
        ...
        const errorMessage =
          error instanceof Error ? error.message : "ToDoリストの取得に失敗しました"
  ...
  const updateTodo = useCallback(
    async (
      ...
      try {
        ...
        setTodos((prev) => prev.map((todo) => (todo.todo_id === todoId ? updatedTodo : todo)))
        return updatedTodo
      } catch (error) {
  ...
  const toggleTodoCompletion = useCallback(async (todoId: number) => {
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : "完了状態の切り替えに失敗しました"
      setError(errorMessage)
      throw error
    }
  }, [])
