...
export const useTodoForm = (options: UseTodoFormOptions = {}) => {
  ...
  const [formData, setFormData] = useState<TodoFormData>({
    ...
    todo_deadline: initialData.todo_deadline ? formatTodoDate(initialData.todo_deadline) : "",
  ...
  const updateField = useCallback(
    (field: keyof TodoFormData, value: any) => {
  ...
  const getSubmitData = useCallback(() => {
    return {
      ...
      formData.todo_deadline !== formatTodoDate(initialData.todo_deadline)
        ? { todo_deadline: formData.todo_deadline }
        : {}),
  ...
  const handleSubmit = useCallback(
    async (submitFunction: (data: any) => Promise<any>) => {
      if (!validate()) {
        return false
      }
